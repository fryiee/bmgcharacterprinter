<template>
  <div class="character">
    <a href="#" @click.prevent="toggleExpand" :class="'w-full noprint px-6 py-2 bg-yellow-400 flex items-center justify-start border-b-2 border-black'+(index === 1 ? ' border-t-2' : '')">
      ...
      <h2 class="text-3xl font-sans ml-2 mb-0 max-w-8xl leading-none overflow-ellipsis overflow-hidden whitespace-nowrap">{{index}}. {{characterAlias}} [{{characterName}}]</h2>
    ...
    <div :class="'character__expander'+(expanded ? ' character__expander--expanded' : '')">
      <div class="flex flex-col character__card__container">
        <a href="#" @click.prevent="$emit('click', character)" class="character__card" :style="backgroundStyle">
          <span class="character__card__version"></span>
          <div class="character__card__banner">
            <div class="character__card__banner__name">
              <h4>{{characterAlias}}</h4>
              <h5>{{characterName}} / {{characterBaseSize}}</h5>
            </div>
...
export default {
  ...
  props: ['character', 'index', 'affiliations', 'traits', 'equipment', 'upgrades', 'weapons', 'crew', 'version', 'eternal', 'showEquipmentCard', 'showWeaponTraitsCard', 'combineAllCards', 'allSelectedCreatedCharacters', 'grantedTraits'],
  data() {
    return {
      icons: [
        'aff_amazons_icon',
  ...
  computed: {
    characterName () {
      if (this.customCharacterData.name) {
        return this.customCharacterData.name
      } else {
        return 'New Name'
      }
    },
    characterAlias () {
      if (this.customCharacterData.alias) {
        return this.customCharacterData.alias
      } else {
        return 'New Alias'
      }
    },
    characterBaseSize () {
      if (this.customCharacterData.bases_size) {
        return this.customCharacterData.bases_size
      } else {
        return 'New Alias'
      }
    },
    backgroundStyle() {
      return 'background-image:url("' + this.customCharacterData.background + '");'
    },
    nowYear () {
      return (new Date()).getUTCFullYear().toString()
    },
    characterAffiliations() {
      if (!this.customCharacterData.affiliations || this.customCharacterData.affiliations.length === 0) {
        return []
      } else {
        const affiliations = []

        this.customCharacterData.affiliations.forEach((affiliation) => {
    ...
    characterRivals() {
      if (!this.customCharacterData.rival_affiliation_ids || this.customCharacterData.rival_affiliation_ids.length === 0) {
        return []
      } else {
        const affiliations = []

        this.customCharacterData.rival_affiliation_ids.forEach((affiliation) => {
          for (let i = 0; i < this.affiliations.length; i++) {
            let existingAffiliation = {...this.affiliations[i]}
    ...
    characterTraits() {
      if (!this.customCharacterData.traits || this.customCharacterData.traits.length === 0) {
        return []
      } else {
        let traits = []

        this.customCharacterData.traits.forEach((trait) => {
    ...
    excludableCreatedCharacterTraits () {
    ...
    availableEquipment() {
      if (!this.equipment || this.equipment.length === 0) {
        ...
        this.equipment.forEach((equipment) => {
          ...
          if (equipment.banned_character_ids.includes(this.customCharacterData.id)) {
            canAdd = false
          }
          ...
          if (equipment.required_rank_ids.length) {
            let inRank = false
            this.customCharacterData.rank_ids.forEach((rankId) => {
          ...
          if (equipment.required_character_ids.length) {
            ...
            if (equipment.required_character_ids.includes(this.customCharacterData.id)) {
              isCreatedCharacter = true
            } else {
              for (let i = 0; i < this.allSelectedCreatedCharacters.length; i++) {
                const existingCreatedCharacter = this.allSelectedCreatedCharacters[i]

                if (equipment.required_character_ids.includes(existingCreatedCharacter.id)) {
                  isCreatedCharacter = true
                }
              }
            }
    ...
    characterUpgrades() {
      if (!this.customCharacterData.upgrade_ids || this.customCharacterData.upgrade_ids.length === 0) {
        return []
      } else {
        const upgrades = []

        this.customCharacterData.upgrade_ids.forEach((upgrade_id) => {
    ...
    characterRanks() {
      if (!this.customCharacterData.rank_ids || this.customCharacterData.rank_ids.length === 0) {
        return []
      } else {
        const ranks = []

        this.customCharacterData.rank_ids.forEach((rank_id) => {
    ...
    characterWeapons() {
      if (!this.customCharacterData.weapon_ids || this.customCharacterData.weapon_ids.length === 0) {
        return []
      } else {
        const weapons = []

        this.customCharacterData.weapon_ids.forEach((weapon_id) => {
